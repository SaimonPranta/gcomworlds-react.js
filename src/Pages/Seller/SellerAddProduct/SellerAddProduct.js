...
const SellerAddProduct = () => {
  ...
  const handleInputValue = (e) => {
    ...
    if (name === "sellerPrice" || name === "mrpProduct") {
      currentInfo[name] = value;
      let price = 0;
      let commissionPercent = productInfo?.mrpProduct ? 12 : 25;
      if (name === "mrpProduct") {
        commissionPercent = value === "true" ? 12 : 25;
      }
      if (name === "sellerPrice") {
        price = Number(value);
      } else if (productInfo?.sellerPrice) {
        price = Number(productInfo?.sellerPrice);
      }

      const webPrice = price + Number(price * Number(commissionPercent / 100));
      if (name === "mrpProduct") {
        const newValue = value === "true" ? true : false;
        setProductInfo({ ...currentInfo, [name]: newValue, price: webPrice });
        return;
      }
      setProductInfo({ ...currentInfo, [name]: value, price: webPrice });
      return;
    }
    currentInfo[name] = value;
    setProductInfo(currentInfo);
  ...
  const handleFormSubmit = (e) => {
    ...
    console.log("hello");
    fetch(
      `${process.env.REACT_APP_SERVER_HOST_URL}/seller/product/add_product`,
      {
        method: "POST",
        headers: {
          ...
          <form onSubmit={(e) => handleFormSubmit(e)}>
            ...
            <div>
              ...
              <input
                ...
                value={productInfo.sellerPrice ? productInfo.sellerPrice : ""}
                onChange={(e) => handleInputValue(e)}
              />
            </div>
            <div>
            ...
            {productInfo?.sealAs?.includes("Wholesale") && (
              <div>
                <label>Wholesale count</label>
                <input
                  type="number"
                  name="WholesaleCount"
                  placeholder="Enter Delivery charge"
                  value={
                    productInfo.WholesaleCount ? productInfo.WholesaleCount : ""
                  }
                  onChange={(e) => handleInputValue(e)}
                />
              </div>
