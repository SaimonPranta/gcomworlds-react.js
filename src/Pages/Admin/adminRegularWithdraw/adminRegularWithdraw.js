...
const AdminRegularWithdraw = () => {
  ...
  useEffect(() => {
    ...
    if (allUser && allUser.length > 0) {
      ...
      allUser.map((user) => {
        user.regularWithdroawHistory?.map((item) => {
          if (item) {
            if (item.approve) {
  ...
  const withdrawRequestApproval = (
    ...
    if (
      ...
      penging = true;
      fetch(`${process.env.REACT_APP_SERVER_HOST_URL}/admin/regular_withdraw_approve`, {
        method: "POST",
        body: JSON.stringify({
          userID,
          requestID,
  ...
  const withdrawRequestDecline = (e, userID, requestID) => {
    ...
    if (!penging && userID && requestID) {
      ...
      fetch(`${process.env.REACT_APP_SERVER_HOST_URL}/admin/regular_withdraw_decline`, {
        method: "POST",
        body: JSON.stringify({
          userID,
          requestID,
  ...
  return (
    <section >
      <div>
        <h3 className="main-title">Regular Withdraw Request</h3>
      </div>
      ...
      <div className="common-table-style">
        <div className="d-flex align-items-center">
          <h4>Regular Withdraw Request History</h4>
        </div>
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th>Phone Number</th>
                      <th>Ammount of Regular Points</th>
                      <th>Ammount of TK</th>
                  ...
                  <tbody>
                    {allUser?.length > 0 &&
                      ...
                      allUser.map((user, index) => {
                        console.log(user?.regularWithdroawHistory.length);
                        return user?.regularWithdroawHistory?.map((items) => {
                          console.log(items);
...
export default AdminRegularWithdraw;
